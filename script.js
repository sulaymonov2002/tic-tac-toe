...
window.onload = () => {
  for (let i = 0; i < allBox.length; i++) {
    //add onclick attribute in all avaiable section's spans
...
function clickedBox(element) {
  if (players.classList.contains("player")) {
    ...
    playerSign = "O"; //player will be O then we'll change the sign
    ...
    selectWinner(); //caling winner function
...
function bot() {
  playerSign = "O";
  let array = []; //creating empty array...we'll store unselected box index in the array
  for (let i = 0; i < allBox.length; i++) {
